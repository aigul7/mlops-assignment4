...
if scenario == 'baseline':
    ...
    current = pd.read_csv(scenario_map[scenario], encoding='latin-1')
    curr_pred_col = [c for c in current.columns if 'PREDICTION' in c.upper()]
    current['prediction'] = current[curr_pred_col[0]].values
...
"@ | Out-File -FilePath "scripts\run_monitoring.py" -Encoding utf8